...
<div class="form_row">
    ...
    <div class="form_rightcol">
        <input class="geo textinput" id="<%=xssAPI.encodeForHTMLAttr(name)%>"  placeholder = "<%=placeHolderTitle%>" name="<%=xssAPI.encodeForHTMLAttr(name)%>" value="" type="password" autocomplete="off" size="<%=xssAPI.encodeForHTMLAttr(cols)%>" <%= width.length() > 0 ? "style=\"width:" + xssAPI.encodeForHTMLAttr(width) + "\"" : "" %> <% if(required){ %> required <% } %> <%if(!minlength.isEmpty()){%>minlength="<%=minlength%>"<%}%> <%if(!maxlength.isEmpty()){%>maxlength="<%=maxlength%>"<%}%>>
    </div>
</div>
<div class="form_row">
    <% LayoutHelper.printTitle(id, confirmTitle, required, hideConfirmTitle, out); %>
    <div class="form_rightcol">
        <input class="geo textinput" placeholder = "<%=placeHolderConfirmTitle%>" name="confirm<%=xssAPI.encodeForHTMLAttr(name)%>" id="confirm<%=xssAPI.encodeForHTMLAttr(name)%>" value="" type="password" autocomplete="off" size="<%=xssAPI.encodeForHTMLAttr(cols)%>" <%= width.length() > 0 ? "style=\"width:" + xssAPI.encodeForHTMLAttr(width) + "\"" : "" %><% if(required){ %> required <% } %> <%if(!minlength.isEmpty()){%>minlength="<%=minlength%>"<%}%> <%if(!maxlength.isEmpty()){%>maxlength="<%=maxlength%>"<%}%>>
    </div>
</div>
<%
    if (key != null && uid != null) {
