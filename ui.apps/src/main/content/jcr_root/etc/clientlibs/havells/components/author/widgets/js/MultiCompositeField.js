...
[{
    ...
    addItem: function(name, o) {
        ...
        //console.log('add item name = ' + name + " items.getCount():  " + this.items.getCount());
        ...
        //console.log('item.processPath(this.path) = ' + this.path);
    ...
    getValue: function() {
        //console.log("MultiCompositeField.getValue was called:");

        var value = new Array();
        this.items.each(function(item, index/*, length*/) {
            //console.log("MultiCompositeField.getValue loop:");

            if (item instanceof CQ.form.MultiCompositeField.Item) {
		//console.log("instanceof CQ.form.MultiCompositeField.Item!:"  + item.constructor);
                //console.log("item.path: " + item.path);

                value[index] = item.getValue();
                for (var n in value[index]) {
			//console.log("!!!n = "  + n);
			if (n.substring(0, 2) == "./")
			{
			//console.log("Another!!");
		...
		//console.log("Not instanceof CQ.form.MultiCompositeField.Item:" + item.constructor + " path:" + item.path);

		if (item instanceof CQ.form.MultiCompositeField.Item) {
			//console.log("instanceof CQ.form.MultiCompositeField.Item!:" + typeof(item));
...
CQ.form.MultiCompositeField.Item = CQ.Ext.extend(CQ.Ext.Panel, {
    ...
    constructor: function(config) {
		...
		//console.log("item constructor f = " + f.rawFieldName);
		//console.log("item constructor f constructor = " + typeof f);

		if (f.rawFieldName.substring(0, 2) == "./")
		{
                var test = config.prefix + config.name + "/" + f.rawFieldName.substring(2,f.rawFieldName.length);
		//console.log("item constructor test = " + test);
		...
		//console.log("item constructor  f.name = " +  f.name);
...
//MT: section for ordering
		...
		//console.log("adding item.rawFieldName: " + item.rawFieldName);

                this.fields.add(item.rawFieldName, item);
            }
            else
            {
		//console.log("!item.rawFieldName");
            }
        }, this);
...
//    },
    ...
    processPath: function(path) {
	//console.log("item processpath = " + path);
    ...
    getValue: function() {
		...
		for (var n in value) {
            ...
            //console.log('item getvalue o[f.rawFieldName] = ' + f.getValue());
    ...
    setValue: function(value) {
        this.fields.each(function(f) {
		//console.log("item setValue f.rawFieldName = " + value[f.rawFieldName]);
