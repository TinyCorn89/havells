 ...
 */
public class HavellsHelper {

    private static final Logger LOGGER = LoggerFactory.getLogger(HavellsHelper.class);
    ...
    public static String getPageTitle(final Page page) {
        if (page != null) {
            ...
            if (isEmpty(title)) {
                return HavellsHelper.getTitle(page);
            }
            return title;
        } else {
   ...
   /* public static int getPageCommentCount(Page page, ResourceResolver resourceResolver) {
    ...
    /**
     * Returns the extension of the image associated with this page.
     * @param   page The page in question
     * @return  String file extension (no "." prefix), or null if no image is referenced
     */
    public static String getPageImageExtension(Page page) {
        if (page != null) {
            Resource image = page.getContentResource().getChild("image");
            if (image != null) {
                ValueMap properties = image.adaptTo(ValueMap.class);
                String fileReference = properties.get("fileReference", "");

                if (fileReference.length() != 0) {
                    // Return the extension
                    return fileReference.substring(fileReference.lastIndexOf('.') + 1);
                }
                else {
                    Resource file = image.getChild("file");
                    if (file != null) {
                        Resource fileJcrContent = file.getChild("jcr:content");
                        if (fileJcrContent != null) {
                            ValueMap fileProperties = fileJcrContent.adaptTo(ValueMap.class);

                            // Default to jpg in the absence of jcr:mimeType
                            String mimeType = fileProperties.get(JcrConstants.JCR_MIMETYPE, "jpg");
